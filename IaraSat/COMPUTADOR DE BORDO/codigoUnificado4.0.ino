...
void loop() 
{
  ...
  Serial.println("======================================\n");
  Serial.println("Digite 1 para ler o MPU6050, 2 para ler o BME280, 3 para ler o DHT22, 4 para ler o GPS");

  // Verifica se tem dado disponível no Serial
  if (Serial.available() > 0) {
    int opcao = Serial.parseInt();
    //Início de Rotina da Comunicação Serial
    switch(opcao) {
      case 1: // Dados de Leitores
        Serial.println("Você escolheu as leituras dos Sensores: ");
      ...
      case 2: // Mecanismo da Antena(Abertura)
        Serial.println("Você escolheu abrir o mecanismo de Antena");
        enviarOrdemslave("Abre Antena", 2); //Envia uma String e um endereço(A ser definido para LoRa)
        aguardarResposta();
        break;

      case 3: // Mecanismo da Antena(Fechamento)
        Serial.println("Você escolheu Fechar o Mecanismo da Antena");
        enviarOrdemslave("Fecha Antena", 3); //Envia uma String e um endereço(A ser definido para LoRa)
        aguardarResposta();
        break;

      case 4: // Condicionamento de Bateria(Suprimento de Energia)
        Serial.println("Você escolheu o condicionamento de bateria");
        enviarOrdemslave("Carregar Bateria", 4);
        aguardarResposta();
        break;

      case 5: //Controle de atitude(Achar azimuth)
        Serial.println("Você escolheu, determinar o azimuth(Sol)");
        enviarOrdemslave("Determinar Azimuth", 5);
        aguardarResposta();
        break;


      case 6: //Controle de Atitude(Orientação)
        Serial.println("Você escolheu, se orientar ao azimuth(Sol)");
        enviarOrdemslave("Orientar para Azimuth", 6);
        aguardarResposta();
        break;
      case 7: //Controle de Atitude(Orientação Dupla)
        Serial.println("Você escolheu, se orientar para dois azimuth(Sol)");
        enviarOrdemslave("Orientar para 2 Azimuth", 7);
        aguardarResposta();
        break;
      default:
...
void leituraMPU6050(sensorsData &dados) 
{
    ...
    Serial.print("Temperatura MPU: "); Serial.print(temp_mpu.temperature); 
    Serial.println(" °C\n");
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssiValue, int8_t snr) 
{
    ...
    {
        //DEPOIS TEM QUE DEFINIR OS COMANDOS VIA RÁDIO   
    }
