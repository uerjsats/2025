...
void setup() {
    Serial.begin(9600);
    gpsSerial.begin(9600, SERIAL_8N1, 45, 46); //Serial do GPS
    controleSerial.begin(9600, SERIAL_8N1, 19, 20); //Serial Controle de Atitude
    raspSerial.begin(9600, SERIAL_8N1, 7, 6); //Serial Raspberry
...
void loop() {
    ...
    if (millis() - lastTxTime >= txInterval && lora_idle) {
        ...
        receberRespostasuprimento();
        receberRespostarasp();
        receberRespostacontrole();
...
//Recebe os dados de suprimento
int receberRespostasuprimento() 
{
...
//Recebe os dados do controle do atitude
int receberRespostacontrole() 
{
    if (!controleSerial.available()) return -1;

    // Lê a linha inteira de uma vez
    String respostaCA = controleSerial.readStringUntil('\n');
    respostaCA.trim();  // Remove \r e espaços

    if (respostaCA.length() == 0) return -1;

    // Verifica se existe o separador
    int idx = respostaCA.indexOf(':');
    if (idx == -1) {
        return -1; // Pacote inválido
    }

    String enderecoCA = respostaCA.substring(0, idx);
    int endereco2 = respostaCA.toInt();

    // Dados depois do endereço
    String dadosCA = respostaCA.substring(idx + 1);
...
//Recebe os dados do raspberry
int receberRespostarasp() 
{
    if (!raspSerial.available()) return -1;

    // Lê a linha inteira de uma vez
    String respostaR = raspSerial.readStringUntil('\n');
    respostaR.trim();  // Remove \r e espaços

    if (respostaR.length() == 0) return -1;

    // Verifica se existe o separador
    int idx = respostaR.indexOf(':');
    if (idx == -1) {
        return -1; // Pacote inválido
    }

    String enderecoRasp = respostaR.substring(0, idx);
    int endereco3 = enderecoRasp.toInt();

    // Dados depois do endereço
    String dadosRasp = respostaR.substring(idx + 1);
...
void enviarOrdemslave(int Buscador) 
{
  switch(Buscador){
    case 0:
        controleSerial.println("0"); //Controle de Atitude pronto
        break;
    case 1:
        controleSerial.println("1"); // Estabilização inicializada
        break;
    
    case 2:
        controleSerial.println("3:23.00 45.00"); //Envia 2 angulos
        break;

    case 3:
        controleSerial.println("3"); //orienta pra luz
        break;
 
    case 4:
        controleSerial.println("4"); //procura o azimuth
        break;
    
    case 5: 
        controleSerial.println("5"); //parar controle de atitude
        break;
    
    case 6: 
        controleSerial.println("6"); //abrir painéis 
        break;

    case 7:
        controleSerial.println("7"); //abrir antena
        break;

    case 8:
        controleSerial.println("8"); //Fecha a antena
        break;
    
    case 9:
        controleSerial.println("9"); //Ajusta o PID
        break;

    case 10:
        controleSerial.println("10"); //Emergência
        break;
    case 12:
        //raspSerial.println("AIS-catcher -S 10110 & python3 /home/uerjsats/codigo_missao/decode_gil.py");
        raspSerial.println("12"); //Missão
        break;
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssiValue, int8_t snr) {
    ...
    if (strcmp(rxpacket, "1") == 0) { //ESTABILIZAR SATÉLITE
        enviarOrdemslave(1);
    }
    else if(strcmp(rxpacket, "2") == 0) //ANGULOS PRO CONTROLE
    {
        enviarOrdemslave(2);
    }
    else if(strcmp(rxpacket, "3") == 0) //ORIENTA PRA LUZ
    {
        enviarOrdemslave(3);
    }
    else if(strcmp(rxpacket, "4") == 0) //PROCURA AZIMUTE
    {
        enviarOrdemslave(4);
    }
    ...
    else if(strcmp(rxpacket, "9") == 0) // AJUSTA O PID
    {
        enviarOrdemslave(9);
    }
    else if(strcmp(rxpacket, "10") == 0) // EMERGÊNCIA
    {
        enviarOrdemslave(10);
    }
    else if(strcmp(rxpacket, "12") == 0) // MISSÃO
    {
        enviarOrdemslave(12);
    }
