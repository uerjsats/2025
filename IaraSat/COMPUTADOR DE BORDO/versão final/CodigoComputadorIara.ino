...
void loop() {
    ...
    if (millis() - lastTxTime >= txInterval && lora_idle) {
        ...
        //Monta o pacote LoRa
        snprintf(&txpacket[2], BUFFER_SIZE - 2, "%u:%.2f:%.2f:%.2f:%.2f:%d:%.6f:%.6f:%.2f:%.2f:%.2f:%s:%s:%s",
                 ...
                 dados.accelX, dados.accelY, dados.accelZ, powerSupplyData.c_str(), ACData.c_str(), MData.c_str());
...
int receberRespostaslave() 
{
  if (Serial.available() > 0) 
  {
    ...
    {
      //Serial.println("Erro ao ler os dados do Slave");
      return -1;
    }
}
