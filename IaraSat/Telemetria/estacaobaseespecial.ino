...
void setup() {
  ...
  myDisplay.clear();
  myDisplay.display();
  myDisplay.drawString(0, 0, "Iniciando...");
  myDisplay.display();
  ...
  delay(1000);
  myDisplay.clear();
  myDisplay.drawString(0, 0, "Aguardando pacotes...");
  myDisplay.display();
}
...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssiValue, int8_t snr) {
  ...
  if (receiver != MY_ADDRESS || sender != DEST_ADDRESS) {
    // Ignora se não for pra mim ou se não veio do destino correto
    lora_idle = true;
    Radio.Rx(0);
    return;
  }
  ...
  myDisplay.clear();
  myDisplay.drawString(0, 0, "Telemetria CUBEDESIGN");
  myDisplay.drawString(0, 10, "Msg:");
  myDisplay.drawString(0, 20, displayMessage);
  myDisplay.drawString(0, 40, "Pkts: " + String(pacoteRecebidoCount));
  myDisplay.display();
